...
console.log('6b. Testnet toggle for existing users')
r = await req('/api/profile/mode', { method: 'POST', token, body: { mode: 'testnet' } })
check('toggle to testnet', r.status === 200 && r.data.profile?.mode === 'testnet', JSON.stringify(r.data))
r = await req('/api/wallet', { token })
check('testnet wallet starts at 0', r.data.balance === 0, `balance=${r.data.balance}`)
r = await req('/api/wallet/deposit', { method: 'POST', token, body: { amount: 500 } })
check('testnet mock deposit credits instantly', r.status === 200 && r.data.balance === 500, JSON.stringify(r.data))
r = await req('/api/bots', { method: 'POST', token, body: { symbol: 'SOLUSDT', strategy: 'momentum', capital: 400 } })
const testnetBotId = r.data.bot?.id
r = await req(`/api/bots/${testnetBotId}/start`, { method: 'POST', token })
check('trade on testnet with mock funds', r.status === 200 && r.data.bot?.status === 'running')
r = await req('/api/portfolio', { token })
check('testnet portfolio scoped', r.status === 200 && r.data.mode === 'testnet' && r.data.balance === 500, JSON.stringify(r.data).slice(0, 120))
r = await req('/api/profile/mode', { method: 'POST', token, body: { mode: 'live' } })
check('toggle back to live', r.status === 200 && r.data.profile?.mode === 'live')
r = await req('/api/wallet', { token })
check('live wallet intact after toggling', r.data.balance === 0, `balance=${r.data.balance}`)
